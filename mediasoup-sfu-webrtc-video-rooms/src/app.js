...
function joinRoom(socket, roomId, username) {
  ...
  const peer = new Peer(socket.id, username);
  room.addPeer(peer);

  // Attach the roomId to the socket
...
io.on('connection', (socket) => {
  const query = url.parse(socket.handshake.headers.referer, true).query;
  const roomId = query.roomId;
  const username = query.username;

  if (!roomId || !username) {
    console.error('Missing roomId or username in query parameters');
    socket.disconnect();
    return;
  }

  if (roomId && username) {
    console.log('Auto-joining room', { roomId, username });

    if (!roomList.has(roomId)) {
      // Optionally create the room if it doesn't exist
      console.log(`Room ${roomId} does not exist. Creating...`);
      const worker = getMediasoupWorker(); // Directly call and get the worker synchronously
